...
{
    return isNonTerminal(name) && nullables.find(name) != nullables.end();
}
...
State::State(const QVector<DottedItem> &kernel): index(0), kernel(kernel), defaultReduce(0)
{
        ...
        for (int i = 0; i < q->labels.size(); ++i) {
            if (G->isNonTerminal(q->labels.at(i))) {
                ...
                for (int j = 0; j < r->labels.size(); ++j) {
                    if (G->isNonTerminal(r->labels.at(j)) && G->isNullable(r->labels.at(j))) {
                    ...
                    foreach (const DottedItem &other, state->items) {
                        if (other != item && state->lookaheads.value(other).contains(la)) {
                            if (fst)
                                fprintf(fp, "\n");
                            fst = false;
                            fprintf(fp, "\treduce/reduce conflict on token %s\n",
                                    qPrintable(la));
                        }
                    }
                }
